...
// 1. To make a function parameter optional, you use the ? after the parameter name. For example:
...
// 2. The optional parameters must appear after the required parameters in the parameter list.
// For example, if you make the b parameter optional, and c parameter required the TypeScript compiler will issue an error:
function multiplyX(a: number, b?: number, c: number): number {
  if (typeof c !== 'undefined') {
    return a * b * c
  }
  return a * b
